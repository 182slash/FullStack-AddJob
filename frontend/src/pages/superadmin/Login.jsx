  ...
  const onSubmit = async (data) => {
    ...
    if (result.success) {
      if (result.role !== 'superadmin') {
        setApiError('Akun ini bukan akun Super Admin.')
        return
      }
    ...
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '40px 16px', background: 'var(--bg)' }}>
      ...
      >
        ...
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', marginBottom: 32, gap: 12 }}>
          ...
          <span style={{ display: 'flex', alignItems: 'center', gap: 6, background: '#fef3c7', color: '#d97706', fontSize: '0.8125rem', fontWeight: 600, padding: '4px 12px', borderRadius: 20 }}>
            <ShieldCheck size={13} /> Super Admin Portal
          </span>
        </div>
        ...
        <div className="card card-body">
          ...
          <p style={{ color: 'var(--muted)', fontSize: '0.9375rem', marginBottom: 28 }}>
            Halaman khusus manajemen tim sales AddJob.
          </p>
